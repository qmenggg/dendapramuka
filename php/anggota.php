            ...
            <tbody>
                ...
                while ($data = mysqli_fetch_array($anggota)) {
                    ...
                    <tr>
                        ...
                        <td><?= $data['jkel'] ?></td>
                        <td>
                            <button class="btn btn-outline-primary btn-sm btn-sm" data-bs-toggle="modal" data-bs-target="#editAnggota_<?= $data['id']; ?>">
                                Edit
                            </button>
...
<?php $anggota = mysqli_query($db, "select * from anggota");
                while ($data = mysqli_fetch_array($anggota)) {?>
            ...
            <form action="proses_edit_anggota.php" method="POST">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <select name="kelas" class="form-select" required>
                            <option value="">-- Pilih Kelas --</option>
                            <option <?php if($data['kelas']== 'BUSANA') echo 'selected';?> value="BUSANA">BUSANA</option>
                            <option <?php if($data['kelas']== 'TJKT1') echo 'selected';?> value="TJKT1">TJKT 1</option>
                            <option <?php if($data['kelas']== 'TJKT2') echo 'selected';?> value="TJKT2">TJKT 2</option>
                            <option <?php if($data['kelas']== 'PPLG') echo 'selected';?> value="PPLG">PPLG</option>
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        <label>Jenis Kelamin</label>
                        <select name="jkel" class="form-control" required>
                            <option <?php if($data['jkel']== 'Laki-laki') echo 'selected';?> value="Lali-laki">Laki-laki</option>
                            <option <?php if($data['jkel']== 'Perempuan') echo 'selected';?> value="Perempuan">Perempuan</option>
                        </select>
                    </div>
